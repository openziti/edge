...
// newListener creates a new xgress edge_wss listener
...
func (listener *listener) Listen(address string, bindHandler xgress.BindHandler) error {
	...
	if err != nil {
		return fmt.Errorf("cannot listen on invalid edge_wss address [%s] (%s)", address, err)
	}

	pfxlog.Logger().WithField("address", addr).Info("starting edge_wss channel listener")

	listener.underlayListener = channel2.NewWssListener(listener.id, addr, listener.options.channelOptions.ConnectOptions, listener.options.serverCert, listener.options.key)
