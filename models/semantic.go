...
const (
	...
	// SemanticOneOf captures enum value "OneOf"
	SemanticOneOf Semantic = "OneOf"
)
...
func init() {
	var res []Semantic
	if err := json.Unmarshal([]byte(`["AllOf","OneOf"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
...
func (m Semantic) validateSemanticEnum(path, location string, value Semantic) error {
	if err := validate.Enum(path, location, value, semanticEnum); err != nil {
		return err
	}
	return nil
}
