...
type DetailGeoRegionURL struct {
	ID strfmt.UUID
...
func (o *DetailGeoRegionURL) Build() (*url.URL, error) {
	...
	var _path = "/geo-regions/{id}"

	id := o.ID.String()
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on DetailGeoRegionURL")
	}
...
func (o *DetailGeoRegionURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on DetailGeoRegionURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on DetailGeoRegionURL")
	}
