...
func (listener *listener) Listen(address string, bindHandler xgress.BindHandler) error {
	...
	pfxlog.Logger().WithField("address", addr).Info("starting wss listener")

	wssListener := channel2.NewWssListener(listener.id, addr, listener.options.Key, listener.options.ServerCert, listener.options.CA)
	if err := wssListener.Listen(); err != nil {
		return err
	}
	accepter := NewAccepter(listener, wssListener, nil)
	go accepter.Run()
